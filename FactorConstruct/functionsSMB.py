...
def getSMB(largeCapSample: pd.DataFrame):
    ...
    largeport = largeCap.groupby(
        'Date')['Return'].mean().reset_index(name='LargePortReturn')
    smallport = smallCap.groupby(
        'Date')['Return'].mean().reset_index(name='SmallPortReturn')
    smallport['SMB'] = smallport['SmallPortReturn'] - \
        largeport['LargePortReturn']
